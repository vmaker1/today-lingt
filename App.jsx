      ...
      <div style={{ width: "100%", maxWidth: 400, minHeight: "100vh", background: T.paper, position: "relative", boxShadow: "0 0 60px rgba(32,42,68,.15)", display: "flex", flexDirection: "column" }}>
        ...
        {toast && (
          <div style={{ position: "absolute", left: "50%", bottom: 100, transform: "translateX(-50%)", background: T.ink, color: "#fff", padding: "10px 16px", borderRadius: 999, display: "flex", alignItems: "center", gap: 8, whiteSpace: "nowrap", boxShadow: "0 8px 24px rgba(32,42,68,.35)", animation: "pop .25s ease", zIndex: 50 }}>
            ...
            <span style={{ fontSize: 13.5, fontWeight: 500 }}>{toast.label} · <b style={{ color: T.gold }}>+{toast.pts}P</b></span>
          </div>
        )}
    ...
    <div>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 12, letterSpacing: 1.5, opacity: .7 }}>
                {new Date().toLocaleDateString("ko-KR", { month: "long", day: "numeric", weekday: "long" })}
              </p>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(255,255,255,.13)", borderRadius: 999, padding: "4px 10px", fontSize: 11.5, marginTop: 7 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <button onClick={() => setOpenNotif(true)} style={{ position: "relative", width: 38, height: 38, borderRadius: 999, background: "rgba(255,255,255,.13)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                {notifCount > 0 && <span style={{ position: "absolute", top: -2, right: -2, minWidth: 18, height: 18, borderRadius: 999, background: T.rose, color: "#fff", fontSize: 10.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px", border: `2px solid ${T.ink}` }}>{notifCount}</span>}
              </button>
          ...
          <h1 style={{ fontFamily: serif, fontSize: 21, fontWeight: 700, margin: "14px 0 0" }}>믿음님, {greet}</h1>
      ...
      <div style={{ padding: "14px 16px 6px" }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", margin: "18px 2px 10px" }}>
          <h2 style={{ fontFamily: serif, fontSize: 18, fontWeight: 700, color: T.ink, margin: 0 }}>오늘의 신앙 훈련</h2>
          <span style={{ fontSize: 11.5, color: T.muted }}>후기를 남기면 신앙일기가 돼요</span>
        </div>
...
function TrainCard({ dim, done, wide, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      background: T.card, borderRadius: 13, padding: "12px 13px",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 9, width: "100%" }}>
        <div style={{ width: 34, height: 34, borderRadius: 9, flexShrink: 0, background: done ? T.sage : `${dim.c}16`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          {done ? <Check size={19} color="#fff" /> : <Ic size={19} color={dim.c} />}
        </div>
        <span style={{ flex: 1, minWidth: 0, fontSize: 16, fontWeight: 700, color: T.ink, letterSpacing: "-0.2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{dim.label}</span>
        <span style={{ fontSize: 11.5, fontWeight: 700, color: done ? T.sage : T.gold, flexShrink: 0 }}>{done ? "완료" : `+${dim.pts}P`}</span>
      </div>
      <p style={{ margin: "8px 0 0", fontSize: 12.5, color: T.muted, lineHeight: 1.35, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{dim.sub}</p>
    </button>
  );
}
    ...
    <div>
      <div style={{ background: `linear-gradient(150deg, #FFFDF7, ${T.goldSoft})`, borderRadius: 16, padding: "14px 16px", border: `1px solid ${T.goldSoft}`, boxShadow: "0 2px 10px rgba(217,164,65,.1)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11.5, fontWeight: 700, color: T.goldDeep }}><Feather size={13} /> 오늘의 말씀 암송</span>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 12, fontWeight: 700, color: T.rose }}><Flame size={13} fill={T.rose} color={T.rose} /> {streak}일</span>
        </div>
        <p style={{ fontFamily: serif, fontSize: 16.5, lineHeight: 1.55, color: T.ink, margin: "0 0 4px" }}>"{verse.text}"</p>
        <p style={{ margin: "0 0 12px", fontSize: 12, color: T.goldDeep, fontWeight: 700 }}>{verse.ref}</p>
        <button onClick={() => setOpen(true)} disabled={done} style={{ width: "100%", padding: "10px 0", borderRadius: 10, fontSize: 13.5, fontWeight: 700, background: done ? "rgba(95,132,104,.14)" : T.ink, color: done ? T.sage : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
...
function MemorizeModal({ verse, onClose, onDone, done }) {
  ...
  return (
    <Sheet onClose={onClose} title={<><Feather size={15} color={T.gold} /> 말씀 암송 연습</>}>
      <p style={{ margin: "0 0 12px", fontSize: 12, color: T.muted }}>가려진 단어를 톡 누르면 살짝 볼 수 있어요.</p>
      <div style={{ background: T.card, borderRadius: 14, border: `1px solid ${T.line}`, padding: "22px 18px", minHeight: 120, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 8px", justifyContent: "center" }}>
          {words.map((w, i) => hidden(i) ? (
            <button key={i} onClick={() => setPeek((s) => new Set(s).add(i))} style={{ fontFamily: serif, fontSize: 18, color: T.gold, borderBottom: `2px solid ${T.gold}`, minWidth: `${Math.max(1, w.length)}ch`, height: 25, background: "rgba(217,164,65,.08)", borderRadius: 3 }}>&nbsp;</button>
          ) : (
            <span key={i} style={{ fontFamily: serif, fontSize: 18, color: T.ink }}>{w}</span>
          ))}
        </div>
        <p style={{ textAlign: "center", margin: "14px 0 0", fontSize: 12, color: T.goldDeep, fontWeight: 700 }}>{verse.ref}</p>
      </div>
      <div style={{ display: "flex", gap: 5, margin: "14px 0" }}>
        {names.map((n, i) => (
          <button key={n} onClick={() => { setLevel(i); setPeek(new Set()); }} style={{ flex: 1, padding: "8px 0", borderRadius: 9, fontSize: 11, fontWeight: 700, background: level === i ? T.ink : T.card, color: level === i ? "#fff" : T.muted, border: `1px solid ${level === i ? T.ink : T.line}` }}>{n}</button>
        ))}
      </div>
      <div style={{ display: "flex", gap: 9 }}>
        ...
        <button onClick={onDone} disabled={done} style={{ flex: 1, padding: "13px 0", borderRadius: 11, fontSize: 14.5, fontWeight: 700, background: done ? T.sageSoft : T.gold, color: done ? T.sage : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
...
function TrainSheet({ dimKey, done, onClose, onComplete }) {
  ...
  return (
    <Sheet onClose={onClose} accent={dim.c} title={<><Ic size={16} color={dim.c} /> {dim.label}</>}>
      <p style={{ margin: "0 0 14px", fontSize: 12.5, color: T.muted }}>{dim.sub}</p>

      <div style={{ marginBottom: 16 }}>{renderContent(dimKey, { openWeb })}</div>

      <p style={{ margin: "0 0 7px", fontSize: 12.5, fontWeight: 700, color: T.ink }}>오늘의 후기</p>
      <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} placeholder={dim.prompt}
        style={{ width: "100%", border: `1px solid ${T.line}`, borderRadius: 12, padding: "11px 13px", fontSize: 13.5, lineHeight: 1.6, color: T.ink, outline: "none", resize: "none", background: T.paper }} />

      <button onClick={() => { onComplete(dimKey, note); onClose(); }} disabled={done}
        style={{ width: "100%", padding: "13px 0", marginTop: 12, borderRadius: 12, fontSize: 14.5, fontWeight: 700, background: done ? T.sageSoft : dim.c, color: done ? T.sage : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
...
function renderContent(key, { openWeb }) {
  ...
  if (key === "praise")
    return (
      <div style={{ display: "grid", gap: 8 }}>
        <p style={{ margin: "0 0 2px", fontSize: 12, color: T.muted }}>곡을 누르면 유튜브에서 들을 수 있어요</p>
        {PRAISE.map((s) => (
          <button key={s.t} onClick={() => openWeb(ytSearch(`${s.t} ${s.a}`), `${s.t} · ${s.a}`)} style={{ display: "flex", alignItems: "center", gap: 11, textAlign: "left", background: T.card, borderRadius: 12, padding: 10, border: `1px solid ${T.line}` }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}><p style={{ margin: 0, fontSize: 13.5, fontWeight: 700, color: T.ink }}>{s.t}</p><p style={{ margin: 0, fontSize: 11.5, color: T.muted }}>{s.a}</p></div>
            <ExternalLink size={15} color={T.muted} style={{ flexShrink: 0 }} />
          </button>
        ))}
        <p style={{ margin: "6px 2px 0", fontSize: 11, color: T.muted, lineHeight: 1.5 }}>* 유튜브는 광고가 있을 수 있어요. 정식 배포 땐 CCM 스트리밍 플레이리스트를 붙일 수 있어요.</p>
      </div>
    );
  if (key === "worship")
    return (
      <div style={{ display: "grid", gap: 12 }}>
        {WORSHIP_INFO.map((w) => (
          <div key={w.d}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 13.5, fontWeight: 700, color: T.ink, display: "flex", alignItems: "center", gap: 7 }}><Church size={15} color={T.goldDeep} /> {w.d}</span>
              <span style={{ fontSize: 12, color: T.muted }}>{w.t}</span>
            </div>
            <div style={{ display: "grid", gap: 6 }}>
              {w.links.map((l) => (
                <button key={l.name} onClick={() => openWeb(ytSearch(l.q), l.name)} style={{ display: "flex", alignItems: "center", gap: 9, textAlign: "left", background: T.card, borderRadius: 11, padding: "10px 12px", border: `1px solid ${T.line}` }}>
                  ...
                  <span style={{ flex: 1, minWidth: 0, fontSize: 13, color: T.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.name}</span>
        ...
        <p style={{ margin: "2px 2px 0", fontSize: 11, color: T.muted, lineHeight: 1.5 }}>* 우리 교회 유튜브 설교 링크로 바꿔 넣을 수 있어요. 아래 후기에 설교 요약을 남겨도 좋아요.</p>
      </div>
    );
  if (key === "practice")
    return (
      <div>
        <div style={{ background: `${T.sage}0F`, borderRadius: 12, padding: "13px 14px", border: `1px solid ${T.sageSoft}`, marginBottom: 12 }}>
          <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: T.sage, marginBottom: 5 }}>예수님이라면 어떻게 하셨을까?</p>
          <p style={{ margin: 0, fontSize: 13, color: T.inkSoft, lineHeight: 1.65, fontFamily: serif }}>{PRACTICE_WWJD}</p>
        </div>
        <p style={{ margin: "0 0 8px", fontSize: 12, color: T.muted }}>오늘 이런 사랑을 실천해볼까요</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
          {PRACTICE_IDEAS.map((p) => <span key={p} style={{ fontSize: 12.5, color: T.sage, background: T.sageSoft, borderRadius: 999, padding: "7px 13px", fontWeight: 500 }}>{p}</span>)}
        </div>
      </div>
    );
  if (key === "growth")
    return (
      <div style={{ display: "grid", gap: 14 }}>
        <div>
          <p style={{ margin: "0 0 7px", fontSize: 12.5, fontWeight: 700, color: T.olive }}>📚 추천 도서</p>
          <div style={{ display: "grid", gap: 6 }}>
            {GROWTH_BOOKS.map((b) => (
              <button key={b.t} onClick={() => openWeb(ytSearch(`${b.t} ${b.a} 책 소개`), b.t)} style={{ display: "flex", alignItems: "center", gap: 9, textAlign: "left", background: T.card, borderRadius: 11, padding: "10px 12px", border: `1px solid ${T.line}` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: T.ink }}>{b.t}</p><p style={{ margin: 0, fontSize: 11, color: T.muted }}>{b.a}</p></div>
                <ExternalLink size={14} color={T.muted} style={{ flexShrink: 0 }} />
              </button>
            ))}
          </div>
        ...
        <div>
          <p style={{ margin: "0 0 7px", fontSize: 12.5, fontWeight: 700, color: T.olive }}>💡 생각해볼 이슈</p>
          <div style={{ display: "grid", gap: 6 }}>
            {GROWTH_NEWS.map((n) => (
              <button key={n.t} onClick={() => openWeb(ytSearch(n.q), n.t)} style={{ display: "flex", alignItems: "center", gap: 9, textAlign: "left", background: T.card, borderRadius: 11, padding: "10px 12px", border: `1px solid ${T.line}` }}>
                ...
                <span style={{ flex: 1, minWidth: 0, fontSize: 13, color: T.ink, lineHeight: 1.4 }}>{n.t}</span>
                <ExternalLink size={14} color={T.muted} style={{ flexShrink: 0 }} />
              </button>
            ))}
          </div>
...
function BibleFinder({ openWeb }) {
  ...
  if (book) {
    ...
    return (
      <div>
        <button onClick={() => setBook(null)} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12.5, color: T.muted, fontWeight: 600, marginBottom: 10 }}><ChevronLeft size={15} /> 성경 목록</button>
        <p style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 700, color: T.ink }}>{name} <span style={{ fontSize: 12, color: T.muted, fontWeight: 400 }}>· 전체 {ch}장 · 읽을 장을 선택하세요</span></p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 6, maxHeight: 200, overflowY: "auto" }}>
          {Array.from({ length: ch }).map((_, i) => (
            <button key={i} onClick={() => openWeb(bibleUrl(name, i + 1), `${name} ${i + 1}장`)} style={{ padding: "9px 0", borderRadius: 8, background: T.card, border: `1px solid ${T.line}`, fontSize: 12.5, fontWeight: 700, color: T.inkSoft }}>{i + 1}</button>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        {[["nt", "신약"], ["ot", "구약"]].map(([k, l]) => (
          <button key={k} onClick={() => setTestament(k)} style={{ flex: 1, padding: "9px 0", borderRadius: 10, fontSize: 13, fontWeight: 700, background: testament === k ? T.ink : T.card, color: testament === k ? "#fff" : T.muted, border: `1px solid ${testament === k ? T.ink : T.line}` }}>{l}</button>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, maxHeight: 220, overflowY: "auto" }}>
        {books.map((b) => (
          <button key={b[0]} onClick={() => setBook(b)} style={{ padding: "10px 4px", borderRadius: 9, background: T.card, border: `1px solid ${T.line}`, fontSize: 12.5, fontWeight: 700, color: T.ink }}>{b[0]}</button>
        ))}
      </div>
      <p style={{ margin: "10px 2px 0", fontSize: 11, color: T.muted }}>장을 고르면 성경 본문 페이지가 프레임 안에서 열려요.</p>
    </div>
  );
}
...
function QTLinks({ openWeb }) {
  return (
    <div style={{ display: "grid", gap: 9 }}>
      <p style={{ margin: "0 0 2px", fontSize: 12, color: T.muted }}>영상은 앱 안에서 재생돼요</p>
      {QT_LINKS.map((l) => (
        <button key={l.name} onClick={() => openWeb(l.url, l.name, l.kind)} style={{ display: "flex", alignItems: "center", gap: 11, textAlign: "left", background: T.card, borderRadius: 12, padding: 11, border: `1px solid ${T.line}` }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ margin: 0, fontSize: 13.5, fontWeight: 700, color: T.ink }}>{l.name}</p>
            <p style={{ margin: "2px 0 0", fontSize: 11.5, color: T.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.desc}</p>
          </div>
          {l.kind === "embed"
            ? <span style={{ fontSize: 10, fontWeight: 700, color: l.c, background: `${l.c}18`, borderRadius: 999, padding: "3px 8px", flexShrink: 0 }}>앱 내 재생</span>
            : <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 10, fontWeight: 700, color: T.muted, flexShrink: 0 }}><ExternalLink size={12} /> 새 탭</span>}
        </button>
      ))}
    </div>
  );
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", justifyContent: "center", background: "rgba(0,0,0,.45)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 400, height: "100%", background: "#fff", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", borderBottom: `1px solid ${T.line}`, flexShrink: 0 }}>
          ...
          <span style={{ flex: 1, minWidth: 0, fontSize: 14, fontWeight: 700, color: T.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{title}</span>
        ...
        {embed ? (
          ...
          <div style={{ flex: 1, background: "#F7F3EC", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, padding: 28, textAlign: "center" }}>
            ...
            <p style={{ margin: 0, fontSize: 14, color: T.ink, fontWeight: 700 }}>{title}</p>
            <p style={{ margin: 0, fontSize: 12.5, color: T.muted, lineHeight: 1.6 }}>이 페이지는 보안 정책상 앱 안에<br />표시할 수 없어요. 새 탭에서 열어보세요.</p>
            <a href={url} target="_blank" rel="noreferrer" style={{ background: T.ink, color: "#fff", padding: "12px 22px", borderRadius: 999, fontSize: 14, fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 7 }}><ExternalLink size={16} /> 새 탭에서 열기</a>
          </div>
        )}
      </div>
    </div>
...
function MiniVerse({ verse }) {
  return (
    <div style={{ background: `${T.gold}0F`, borderRadius: 12, padding: "12px 14px", border: `1px solid ${T.goldSoft}` }}>
      <p style={{ fontFamily: serif, fontSize: 15, lineHeight: 1.55, color: T.ink, margin: 0 }}>"{verse.text}"</p>
      <p style={{ margin: "5px 0 0", fontSize: 11.5, color: T.goldDeep, fontWeight: 700 }}>{verse.ref}</p>
    </div>
  );
}
          ...
          groups.map(([date, entries]) => (
            <div key={date} style={{ marginBottom: 22 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 11 }}>
                <span style={{ fontFamily: serif, fontSize: 15, fontWeight: 700, color: T.ink }}>{date}</span>
                {date === today && <span style={{ fontSize: 10.5, fontWeight: 700, color: "#fff", background: T.gold, borderRadius: 999, padding: "2px 8px" }}>오늘</span>}
                  ...
                  return (
                    <div key={e.id} style={{ background: T.card, borderRadius: 13, padding: "12px 14px", border: `1px solid ${T.line}`, borderLeft: `3px solid ${d.c}` }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: e.note ? 7 : 0 }}>
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 700, color: d.c }}><Ic size={14} /> {d.label}</span>
                        <span style={{ fontSize: 11, color: T.muted }}>{e.time}</span>
                      </div>
                      {e.note ? <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.65, color: T.inkSoft }}>{e.note}</p>
                        : <p style={{ margin: 0, fontSize: 12.5, color: T.muted, fontStyle: "italic" }}>후기 없이 완료</p>}
                    </div>
                  );
                })}
              </div>
  ...
  return (
    <div>
      <div style={{ padding: "40px 16px 12px", display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontFamily: serif, fontSize: 25, fontWeight: 700, color: T.ink, margin: 0 }}>함께</h1>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: T.muted }}>지체들과 하루를 나누고 서로를 세워요</p>
        </div>
        <button onClick={() => setInvite(true)} style={{ display: "inline-flex", alignItems: "center", gap: 5, background: T.ink, color: "#fff", borderRadius: 999, padding: "8px 13px", fontSize: 12.5, fontWeight: 700 }}>
          <UserPlus size={14} /> 초대
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, padding: "0 16px 4px", position: "sticky", top: 0, background: T.paper, zIndex: 5 }}>
        {tabs.map((t) => (
          <button key={t.k} onClick={() => setSub(t.k)} style={{ flex: 1, padding: "9px 0", borderRadius: 999, fontSize: 13, fontWeight: 700, background: sub === t.k ? T.ink : "transparent", color: sub === t.k ? "#fff" : T.muted, border: `1px solid ${sub === t.k ? T.ink : T.line}`, position: "relative" }}>
            {t.label}
            {t.badge > 0 && <span style={{ position: "absolute", top: 4, right: 12, minWidth: 15, height: 15, borderRadius: 999, background: T.rose, color: "#fff", fontSize: 9.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>{t.badge}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: "0 16px" }}>
        {!writing ? (
          <button onClick={() => setWriting(true)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 11, background: T.card, borderRadius: 14, padding: 13, border: `1px solid ${T.line}`, marginBottom: 16, textAlign: "left" }}>
            <Avatar init="믿" c={T.gold} /><span style={{ flex: 1, color: T.muted, fontSize: 13.5 }}>오늘 어떤 은혜가 있었나요?</span><PenLine size={17} color={T.gold} />
          </button>
        ) : (
          <div style={{ background: T.card, borderRadius: 14, padding: 14, border: `1px solid ${T.gold}`, marginBottom: 16, animation: "rise .3s ease" }}>
            <textarea value={text} onChange={(e) => setText(e.target.value)} autoFocus rows={4} placeholder="오늘 묵상한 말씀, 마음에 남은 감정, 기도 제목을 나눠보세요."
              style={{ width: "100%", border: "none", outline: "none", resize: "none", fontSize: 14, lineHeight: 1.6, color: T.ink, background: "transparent" }} />
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8, borderTop: `1px solid ${T.line}`, paddingTop: 11 }}>
              <button onClick={() => { setWriting(false); setText(""); }} style={{ fontSize: 13, color: T.muted, fontWeight: 500 }}>취소</button>
              <button onClick={post} style={{ background: T.ink, color: "#fff", padding: "8px 16px", borderRadius: 999, fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}><Send size={13} /> 올리기 · +10P</button>
            </div>
          </div>
        )}
          ...
          {posts.map((p) => (
            <div key={p.id} style={{ background: T.card, borderRadius: 16, padding: 15, border: `1px solid ${T.line}`, boxShadow: "0 1px 6px rgba(32,42,68,.03)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <div><p style={{ margin: 0, fontSize: 13.5, fontWeight: 700, color: T.ink }}>{p.name}</p><p style={{ margin: 0, fontSize: 11, color: T.muted, display: "flex", alignItems: "center", gap: 4 }}><Clock size={10} /> {p.time}</p></div>
              </div>
              <p style={{ margin: "0 0 12px", fontSize: 14, lineHeight: 1.65, color: T.inkSoft }}>{p.text}</p>
              <button onClick={() => amen(p.id)} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 700, color: p.amened ? T.rose : T.muted }}><Heart size={15} fill={p.amened ? T.rose : "none"} color={p.amened ? T.rose : T.muted} /> 아멘 {p.amen}</button>
            </div>
          ))}
        </div>
      </div>
...
function Rooms({ rooms, setRooms, award }) {
  ...
  return (
    <div style={{ padding: "0 16px" }}>
      <button onClick={() => setCreating(true)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, background: T.card, border: `1px dashed ${T.gold}`, borderRadius: 14, padding: 13, marginBottom: 14, fontSize: 13.5, fontWeight: 700, color: T.goldDeep }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: T.ink }}>{r.name}</p>
                  {r.mine && <span style={{ fontSize: 10, fontWeight: 700, color: T.sage, background: T.sageSoft, borderRadius: 999, padding: "1px 7px" }}>참여중</span>}
                </div>
                <p style={{ margin: "3px 0 0", fontSize: 12, color: T.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.desc}</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                {r.unread > 0 && <span style={{ minWidth: 18, height: 18, borderRadius: 999, background: T.rose, color: "#fff", fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 5px" }}>{r.unread}</span>}
                <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 12, color: T.muted }}><Users size={13} /> {r.members}</span>
              </div>
            </div>
          </button>
        ))}
  ...
  return (
    <div>
      <div style={{ padding: "0 16px 12px" }}>
        <button onClick={onBack} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, color: T.muted, fontWeight: 500, marginBottom: 10 }}><ChevronLeft size={16} /> 방 목록</button>
        <div style={{ background: `linear-gradient(150deg, ${T.ink}, #3A335E)`, borderRadius: 16, padding: "16px 18px", color: "#fff", position: "relative", overflow: "hidden" }}>
          <StarField faint />
          <div style={{ position: "relative", zIndex: 2 }}>
            <p style={{ margin: 0, fontFamily: serif, fontSize: 19, fontWeight: 700 }}>{room.name}</p>
            <p style={{ margin: "3px 0 10px", fontSize: 12, opacity: .8 }}>{room.desc}</p>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, background: "rgba(255,255,255,.14)", borderRadius: 999, padding: "3px 10px" }}><Users size={12} /> {room.members}명 참여</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 16px" }}>
        <div style={{ display: "flex", gap: 8, background: T.card, borderRadius: 14, padding: 11, border: `1px solid ${T.line}`, marginBottom: 14 }}>
          <input value={text} onChange={(e) => setText(e.target.value)} placeholder="이 방에 오늘의 은혜를 나눠요" style={{ flex: 1, border: "none", outline: "none", fontSize: 13.5, color: T.ink, background: "transparent" }} />
          <button onClick={send} style={{ background: T.ink, color: "#fff", borderRadius: 999, padding: "0 14px", fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}><Send size={13} /> 나눔</button>
        </div>
        <div style={{ display: "grid", gap: 11, paddingBottom: 8 }}>
          {room.feed.length === 0 ? <Empty icon={DoorOpen} text={"아직 나눔이 없어요.\n첫 나눔을 남겨보세요."} /> : room.feed.map((p) => (
            <div key={p.id} style={{ background: T.card, borderRadius: 14, padding: 14, border: `1px solid ${T.line}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 8 }}>
                ...
                <div><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: T.ink }}>{p.name}</p><p style={{ margin: 0, fontSize: 10.5, color: T.muted }}>{p.time}</p></div>
              </div>
              <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: T.inkSoft }}>{p.text}</p>
            </div>
          ))}
        </div>
      </div>
...
function CreateRoomSheet({ onClose, onCreate }) {
  ...
  return (
    <Sheet onClose={onClose} accent={T.violet} title={<><DoorOpen size={16} color={T.violet} /> 새로운 방 만들기</>}>
      <p style={{ margin: "0 0 14px", fontSize: 12.5, color: T.muted }}>함께 신앙을 나눌 방을 만들어 친구를 초대해요.</p>
      ...
      <button onClick={() => name.trim() && onCreate(name.trim(), desc.trim())} disabled={!name.trim()} style={{ width: "100%", padding: "13px 0", marginTop: 6, borderRadius: 12, fontSize: 14.5, fontWeight: 700, background: name.trim() ? T.violet : T.line, color: name.trim() ? "#fff" : T.muted, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
        <Plus size={15} /> 방 만들기
      </button>
    </Sheet>
  );
        ...
        {threads.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => { setOpen(t.id); setThreads((ts) => ts.map((x) => x.id === t.id ? { ...x, unread: 0 } : x)); }} style={{ width: "100%", textAlign: "left", display: "flex", alignItems: "center", gap: 12, background: T.card, borderRadius: 14, padding: 13, border: `1px solid ${T.line}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: T.ink }}>{t.name}</p>
                <p style={{ margin: "2px 0 0", fontSize: 12, color: T.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{last.me ? "나 · " : ""}{last.text}</p>
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <p style={{ margin: 0, fontSize: 10.5, color: T.muted }}>{last.time}</p>
                {t.unread > 0 && <span style={{ display: "inline-block", marginTop: 4, minWidth: 17, height: 17, borderRadius: 999, background: T.rose, color: "#fff", fontSize: 10, fontWeight: 700, lineHeight: "17px" }}>{t.unread}</span>}
              </div>
            </button>
          );
        })}
  ...
  return (
    <div style={{ padding: "0 16px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, paddingBottom: 12, borderBottom: `1px solid ${T.line}`, marginBottom: 14 }}>
        ...
        <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: T.ink }}>{thread.name}</p>
      </div>
        ...
        {thread.msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.me ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: "75%" }}>
              <div style={{ background: m.me ? T.ink : T.card, color: m.me ? "#fff" : T.ink, border: m.me ? "none" : `1px solid ${T.line}`, borderRadius: m.me ? "14px 14px 4px 14px" : "14px 14px 14px 4px", padding: "10px 13px", fontSize: 13.5, lineHeight: 1.55 }}>{m.text}</div>
              <p style={{ margin: "3px 6px 0", fontSize: 10, color: T.muted, textAlign: m.me ? "right" : "left" }}>{m.time}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, position: "sticky", bottom: 8, background: T.paper, paddingTop: 6 }}>
        <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder="쪽지를 입력하세요" style={{ flex: 1, border: `1px solid ${T.line}`, borderRadius: 999, padding: "11px 15px", fontSize: 13.5, color: T.ink, outline: "none", background: T.card }} />
  ...
  return (
    <Sheet onClose={onClose} accent={T.rose} title={<><Bell size={16} color={T.rose} /> 알림</>}>
      {items.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 9 }}>
          <p style={{ margin: "0 0 2px", fontSize: 12, color: T.muted }}>읽지 않은 소식 {items.length}건</p>
          {items.map((it) => {
            const Ic = it.icon;
            return (
              <button key={it.id} onClick={onGo} style={{ width: "100%", textAlign: "left", display: "flex", alignItems: "center", gap: 11, background: T.card, borderRadius: 13, padding: "12px 13px", border: `1px solid ${T.line}` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: 13.5, fontWeight: 700, color: T.ink }}>{it.title}</p>
                  <p style={{ margin: "2px 0 0", fontSize: 12, color: T.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.text}</p>
                </div>
                <span style={{ fontSize: 10.5, fontWeight: 700, color: it.kind === "room" ? T.violet : T.rose, background: it.kind === "room" ? `${T.violet}12` : `${T.rose}12`, borderRadius: 999, padding: "3px 8px", flexShrink: 0 }}>{it.kind === "room" ? "방" : "쪽지"}</span>
              </button>
            );
          })}
          <button onClick={onGo} style={{ width: "100%", marginTop: 4, padding: "12px 0", borderRadius: 12, fontSize: 13.5, fontWeight: 700, background: T.ink, color: "#fff" }}>함께 탭에서 보기</button>
        </div>
      )}
    </Sheet>
  );
...
function InviteSheet({ onClose, share }) {
  ...
  return (
    <Sheet onClose={onClose} accent={T.gold} title={share ? <><Share2 size={16} color={T.gold} /> 앱 공유하기</> : <><UserPlus size={16} color={T.gold} /> 친구 초대하기</>}>
      <p style={{ margin: "0 0 16px", fontSize: 12.5, color: T.muted, lineHeight: 1.6 }}>
      ...
      <div style={{ background: `linear-gradient(150deg, #FFFDF7, ${T.goldSoft})`, borderRadius: 16, padding: "18px 16px", border: `1px solid ${T.goldSoft}`, textAlign: "center", marginBottom: 16 }}>
        <p style={{ margin: 0, fontSize: 11.5, color: T.goldDeep, fontWeight: 700 }}>나의 초대 코드</p>
        <p style={{ margin: "6px 0 0", fontFamily: serif, fontSize: 30, fontWeight: 700, letterSpacing: 4, color: T.ink }}>BELIEVE24</p>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.card, borderRadius: 12, padding: "11px 14px", border: `1px solid ${T.line}`, marginBottom: 16 }}>
        <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, color: T.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{link}</span>
        <button onClick={copy} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 700, color: copied ? T.sage : T.ink, flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {opts.map((o) => (
          <button key={o.label} onClick={o.label === "링크 복사" ? copy : undefined} style={{ flex: 1, padding: "12px 0", borderRadius: 12, background: o.c, color: o.t, fontSize: 12.5, fontWeight: 700 }}>{o.label}</button>
        ))}
      </div>
      <p style={{ margin: "12px 2px 0", fontSize: 11, color: T.muted, textAlign: "center" }}>* 카카오톡·문자 연동은 실제 배포 시 연결돼요</p>
    </Sheet>
  );
}
    ...
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        <div style={{ background: `linear-gradient(160deg, ${T.ink}, #3A335E)`, borderRadius: 18, padding: "22px 20px", color: "#fff", marginBottom: 16, position: "relative", overflow: "hidden" }}>
          <StarField faint />
          <div style={{ position: "relative", zIndex: 2 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3 }}><span style={{ fontSize: 16 }}>{current.emoji}</span><span style={{ fontSize: 12.5, color: T.gold, fontWeight: 700, letterSpacing: .5 }}>{current.stage} · {current.label}</span></div>
            <div style={{ fontFamily: serif, fontSize: 38, fontWeight: 700, lineHeight: 1 }}>{points}<span style={{ fontSize: 16, marginLeft: 4, opacity: .7 }}>P</span></div>
            <div style={{ marginTop: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, opacity: .8, marginBottom: 6 }}><span>다음 · {current.label}</span><span>{points >= current.pt ? "달성 ✦" : `${current.pt - points}P 남음 · 꾸준함 ${current.day}`}</span></div>
        ...
        <div style={{ background: T.card, borderRadius: 16, padding: 16, border: `1px solid ${T.line}`, marginBottom: 16 }}>
          <p style={{ margin: "0 0 13px", fontSize: 13, fontWeight: 700, color: T.ink }}>이번 주 활동</p>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 9, height: 80 }}>
            {week.map((v, i) => (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                ...
                <span style={{ fontSize: 10, color: T.muted }}>{["월", "화", "수", "목", "금", "토", "일"][i]}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <button onClick={() => setFruitOpen(true)} style={{ width: "100%", textAlign: "left", background: `linear-gradient(150deg, #F6F1FA, #EFE7F5)`, borderRadius: 16, padding: 16, border: `1px solid #E3DAF0`, marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 13, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0, boxShadow: "0 2px 8px rgba(124,107,176,.2)" }}>🍇</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ margin: 0, fontSize: 14.5, fontWeight: 700, color: T.ink }}>성령의 아홉 열매 도전</p>
            <p style={{ margin: "2px 0 6px", fontSize: 11.5, color: T.violet }}>열매를 하나씩 키워 뱃지를 모아요 · {earnedFruits.length}/9 수확</p>
            <div style={{ display: "flex", gap: 3 }}>
              {FRUITS.map((f) => (
                <span key={f.key} style={{ fontSize: 13, opacity: earnedFruits.includes(f.key) ? 1 : .28, filter: earnedFruits.includes(f.key) ? "none" : "grayscale(1)" }}>{f.emoji}</span>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: T.card, borderRadius: 16, padding: "16px 16px 6px", border: `1px solid ${T.line}`, marginBottom: 16 }}>
          <p style={{ margin: "0 0 4px", fontSize: 13, fontWeight: 700, color: T.ink }}>믿음의 성장 여정</p>
          <p style={{ margin: "0 0 14px", fontSize: 11.5, color: T.muted }}>{growingFruit ? `지금 키우는 열매 · ${FRUIT[growingFruit].emoji} ${FRUIT[growingFruit].name}` : "밭을 고르고 씨를 뿌려, 자라 열매 맺기까지"}</p>
          {STAGES.map((s, si) => {
            ...
            return (
              <div key={s.stage} style={{ display: "flex", gap: 11, marginBottom: 14 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ width: 34, height: 34, borderRadius: 999, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, background: allDone ? T.sage : isCurrent ? T.goldSoft : "#F1EDE3", border: isCurrent ? `2px solid ${T.gold}` : "none", opacity: started ? 1 : .5 }}>
                ...
                <div style={{ flex: 1, paddingBottom: 2 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: started ? T.ink : T.muted }}>{s.stage}</span>
                    ...
                    {s.steps.map((st) => {
                      ...
                      return (
                        <div key={st.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 12.5, color: d ? T.ink : T.muted, fontWeight: d ? 600 : 400 }}>{st.label}</span>
                          <span style={{ fontSize: 10.5, color: T.muted, marginLeft: "auto" }}>누적 {st.pt}P · 꾸준함 {st.day}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        <p style={{ margin: "0 0 11px", fontSize: 13, fontWeight: 700, color: T.ink }}>적립 내역</p>
        {log.length === 0 ? (
            ...
            {log.map((e) => (
              <div key={e.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: T.card, borderRadius: 11, padding: "11px 14px", border: `1px solid ${T.line}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                  ...
                  <div><p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: T.ink }}>{e.label}</p><p style={{ margin: 0, fontSize: 10.5, color: T.muted }}>{e.time}</p></div>
                </div>
                <span style={{ fontSize: 13.5, fontWeight: 700, color: T.sage }}>+{e.pts}P</span>
              </div>
            ))}
          </div>
        )}
    ...
    <Sheet onClose={onClose} accent={T.violet} title={<>🍇 성령의 아홉 열매</>}>
      ...
      {allDone && (
        <div style={{ background: `linear-gradient(150deg, ${T.gold}, ${T.goldDeep})`, borderRadius: 16, padding: "20px 16px", textAlign: "center", color: "#fff", marginBottom: 16 }}>
          ...
          <p style={{ margin: "6px 0 2px", fontFamily: serif, fontSize: 18, fontWeight: 700 }}>성령의 아홉 열매 완성!</p>
          <p style={{ margin: 0, fontSize: 12, opacity: .9 }}>사랑·희락·화평·인내·자비·양선·충성·온유·절제를 모두 거두었어요 ✦</p>
        </div>
      )}

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <span style={{ fontSize: 12.5, fontWeight: 700, color: T.ink, whiteSpace: "nowrap" }}>수확 {earned.size} / 9</span>
      ...
      {growing && (
        <div style={{ background: `${growing.c}0F`, borderRadius: 16, padding: 16, border: `1px solid ${growing.c}33`, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <span style={{ fontSize: 26 }}>{growing.emoji}</span>
            <div>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: T.ink }}>{growing.name} 열매 키우는 중</p>
              <p style={{ margin: "1px 0 0", fontSize: 11.5, color: T.muted }}>물과 양분을 주고 수확해요</p>
            </div>
          </div>
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {GROW_STEPS.map((s, i) => {
              ...
              return (
                <div key={s} style={{ flex: 1, textAlign: "center" }}>
                  ...
                  <span style={{ fontSize: 10.5, fontWeight: now ? 700 : 400, color: passed || now ? growing.c : T.muted }}>{s}</span>
                </div>
              );
            })}
          </div>
          {growStep < 2 ? (
            <button onClick={growAction} style={{ width: "100%", padding: "12px 0", borderRadius: 12, fontSize: 14, fontWeight: 700, background: growing.c, color: "#fff" }}>
              {growStep === 0 ? "💧 물 주기" : "🍃 양분 주기"}
            </button>
          ) : (
            <button onClick={harvestFruit} style={{ width: "100%", padding: "12px 0", borderRadius: 12, fontSize: 14, fontWeight: 700, background: T.gold, color: "#fff" }}>
      ...
      {!growing && !allDone && (
        <p style={{ margin: "0 0 10px", fontSize: 12.5, color: T.violet, fontWeight: 600, textAlign: "center" }}>키울 열매를 하나 선택하세요 👇</p>
      )}
        ...
        {FRUITS.map((f) => {
          ...
          return (
            <button key={f.key} onClick={() => selectable && selectFruit(f.key)} disabled={!selectable}
              ...
              <span style={{ fontSize: 24, filter: isEarned || isGrowing ? "none" : "grayscale(.4)" }}>{f.emoji}</span>
              <span style={{ fontSize: 12.5, fontWeight: 700, color: isEarned || isGrowing ? T.ink : T.muted }}>{f.name}</span>
              {isEarned ? (
                ...
                <span style={{ fontSize: 9.5, fontWeight: 700, color: f.c, background: `${f.c}1A`, borderRadius: 999, padding: "1px 7px" }}>키우는 중</span>
              ) : selectable ? (
                <span style={{ fontSize: 9.5, color: T.violet, fontWeight: 600 }}>선택하기</span>
              ) : (
                <span style={{ fontSize: 9.5, color: T.muted }}>대기</span>
              )}
            </button>
          );
        })}
  ...
  return (
    <div>
      <div style={{ background: `linear-gradient(170deg, ${T.ink}, #3A335E)`, padding: "40px 24px 28px", color: "#fff", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <StarField />
        <div style={{ position: "relative", zIndex: 2 }}>
          <div style={{ width: 66, height: 66, borderRadius: 999, background: T.gold, margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 27, fontWeight: 700, color: T.ink, fontFamily: serif }}>{user.name[0]}</div>
          <h2 style={{ fontFamily: serif, fontSize: 21, margin: 0 }}>{user.name}님</h2>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 7, background: "rgba(255,255,255,.14)", padding: "4px 13px", borderRadius: 999, fontSize: 12 }}><ShieldCheck size={13} color={T.gold} /> 본인인증 완료 · {st.emoji} {st.stage}</div>
        </div>
      </div>
      <div style={{ padding: 16 }}>
        <button onClick={() => setInvite(true)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, background: `linear-gradient(150deg, #FFFDF7, ${T.goldSoft})`, border: `1px solid ${T.goldSoft}`, borderRadius: 14, padding: 15, marginBottom: 16, textAlign: "left" }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ margin: 0, fontSize: 14.5, fontWeight: 700, color: T.ink }}>친구 초대하기</p>
            <p style={{ margin: "2px 0 0", fontSize: 12, color: T.goldDeep }}>함께 매일의 신앙 훈련을 시작해요</p>
          </div>
        ...
        <button onClick={onOut} style={{ width: "100%", padding: "12px 0", borderRadius: 11, fontSize: 13.5, fontWeight: 700, color: T.rose, background: T.card, border: `1px solid ${T.line}` }}>로그아웃</button>
      </div>
      {invite && <InviteSheet onClose={() => setInvite(false)} />}
    </div>
  );
...
function InfoRow({ icon: Icon, label, value, last }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 13, padding: "14px 16px", borderBottom: last ? "none" : `1px solid ${T.line}` }}>
      ...
      <div style={{ flex: 1 }}><p style={{ margin: 0, fontSize: 11, color: T.muted }}>{label}</p><p style={{ margin: "1px 0 0", fontSize: 14, color: T.ink, fontWeight: 500 }}>{value}</p></div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ padding: "0 16px" }}>
        <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
          {steps.map((s, i) => (
            <div key={s} style={{ flex: 1 }}>
              ...
              <p style={{ margin: "5px 0 0", fontSize: 10, textAlign: "center", color: i <= step ? T.ink : T.muted, fontWeight: i === step ? 700 : 400 }}>{s}</p>
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 9, background: T.goldSoft, borderRadius: 11, padding: "11px 13px", marginBottom: 18 }}>
          ...
          <p style={{ margin: 0, fontSize: 11.5, color: T.ink, lineHeight: 1.5 }}>이메일·휴대폰·생년월일로 본인인증을 진행해요. <b>지금은 데모</b>라 인증번호에 아무 숫자나 넣어도 통과돼요.</p>
        </div>
            ...
            <div style={{ background: T.card, borderRadius: 14, border: `1px solid ${T.line}`, padding: 16, marginBottom: 16 }}>
              {["만 14세 이상이며 서비스 이용약관에 동의합니다.", "개인정보 수집·이용에 동의합니다.", "본인 명의의 정보로 인증함에 동의합니다."].map((t, i) => (
                <label key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "9px 0", borderBottom: i < 2 ? `1px solid ${T.line}` : "none", cursor: "pointer" }}>
                  ...
                  <span style={{ fontSize: 13, color: T.inkSoft, lineHeight: 1.5 }}>{t}</span>
                </label>
              ))}
            </div>
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ background: T.sageSoft, borderRadius: 11, padding: "11px 13px", margin: "4px 0 16px", display: "flex", gap: 8, alignItems: "center" }}><ShieldCheck size={16} color={T.sage} /><span style={{ fontSize: 12, color: "#3E5A44", fontWeight: 500 }}>이메일·휴대폰 인증 완료됨</span></div>
            <div style={{ display: "flex", gap: 9 }}>
  ...
  return (
    <div>
      <div style={{ marginBottom: 13 }}>
        <p style={{ margin: "0 0 7px", fontSize: 12.5, fontWeight: 700, color: T.ink }}>{label}</p>
        <div style={{ display: "flex", gap: 8 }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 9, background: T.card, border: `1px solid ${T.line}`, borderRadius: 11, padding: "0 13px" }}>
            ...
            <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} style={{ flex: 1, border: "none", outline: "none", padding: "12px 0", fontSize: 14, color: T.ink, background: "transparent" }} />
          </div>
          <button onClick={onSend} disabled={!value} style={{ padding: "0 15px", borderRadius: 11, background: value ? T.ink : T.line, color: value ? "#fff" : T.muted, fontSize: 12.5, fontWeight: 700, whiteSpace: "nowrap" }}>{sent ? "재발송" : "인증요청"}</button>
        </div>
      </div>
      {sent && (
        <div style={{ marginBottom: 16, animation: "rise .3s ease" }}>
          <div style={{ display: "flex", gap: 8 }}>
            <input value={code} onChange={(e) => onCode(e.target.value)} placeholder="인증번호 6자리" inputMode="numeric" style={{ flex: 1, border: `1px solid ${ok ? T.sage : T.line}`, outline: "none", borderRadius: 11, padding: "12px 13px", fontSize: 14, color: T.ink, background: T.card, letterSpacing: 2 }} />
            <button onClick={onVerify} disabled={!code} style={{ padding: "0 15px", borderRadius: 11, background: ok ? T.sage : (code ? T.gold : T.line), color: code || ok ? "#fff" : T.muted, fontSize: 12.5, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>{ok ? <><Check size={14} /> 완료</> : "확인"}</button>
          </div>
          {ok && <p style={{ margin: "7px 2px 0", fontSize: 11.5, color: T.sage, fontWeight: 500, display: "flex", alignItems: "center", gap: 5 }}><ShieldCheck size={12} /> 인증되었어요</p>}
        </div>
      )}
  ...
  return (
    <div style={{ marginBottom: 13 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 9, background: T.card, border: `1px solid ${T.line}`, borderRadius: 11, padding: "0 13px" }}>
        ...
        <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} style={{ flex: 1, border: "none", outline: "none", padding: "12px 0", fontSize: 14, color: T.ink, background: "transparent" }} />
      </div>
    </div>
  );
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(32,42,68,.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 45 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 400, maxHeight: "88vh", overflowY: "auto", background: T.paper, borderRadius: "22px 22px 0 0", padding: "20px 18px 26px", animation: "rise .3s ease", borderTop: accent ? `3px solid ${accent}` : "none" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontFamily: serif, fontSize: 17, fontWeight: 700, color: T.ink }}>{title}</span>
...
function Header({ title, subtitle }) {
  return (
    <div style={{ padding: "40px 16px 16px" }}>
      <h1 style={{ fontFamily: serif, fontSize: 25, fontWeight: 700, color: T.ink, margin: 0 }}>{title}</h1>
      <p style={{ margin: "4px 0 0", fontSize: 13, color: T.muted }}>{subtitle}</p>
    </div>
  );
}
function Empty({ icon: Icon, text }) {
  return (
    <div style={{ background: T.card, borderRadius: 14, padding: "30px 20px", textAlign: "center", border: `1px dashed ${T.line}` }}>
      ...
      <p style={{ margin: 0, fontSize: 13, color: T.muted, lineHeight: 1.6, whiteSpace: "pre-line" }}>{text}</p>
    </div>
  );
}
function Avatar({ init, c }) {
  return <div style={{ width: 38, height: 38, borderRadius: 999, background: `${c}22`, color: c, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, fontFamily: serif, flexShrink: 0 }}>{init}</div>;
}
function Tag({ children, c, bg }) {
  return <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 10, fontWeight: 700, color: c, background: bg, borderRadius: 999, padding: "2px 8px" }}>{children}</span>;
}
function NextBtn({ children, onClick, disabled }) {
  return <button onClick={onClick} disabled={disabled} style={{ flex: 1, padding: "14px 0", borderRadius: 12, fontSize: 14.5, fontWeight: 700, background: disabled ? T.line : T.ink, color: disabled ? T.muted : "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>{children} <ChevronRight size={16} /></button>;
}
      ...
      {items.map((it) => {
        ...
        return (
          <button key={it.k} onClick={() => setTab(it.k)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "3px 0" }}>
            ...
            <span style={{ fontSize: 10, fontWeight: on ? 700 : 500, color: on ? T.ink : T.muted }}>{it.label}</span>
          </button>
        );
      })}
    </div>
